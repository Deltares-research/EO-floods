...
class FloodMap:
    ...
    def available_data(self) -> None:
        ...
        of images, the timestamp of the images, and a quality score in percentage of the selected
            datasets.
        """
    ...
    def preview_data(
        ...
        **kwargs: dict[str, Any],
        ...
        This can be used to visually check if the quality of the data is sufficient for further
            processing to flood maps. The data can be filtered based on date and dataset name.
...
def _dataset_error(dataset_name: str) -> None:
    err_msg = (f"Dataset '{dataset_name}' not recognized. Supported datasets "
               f"are: {','.join(list(DATASETS.keys()))}")
    raise ValueError(err_msg)
